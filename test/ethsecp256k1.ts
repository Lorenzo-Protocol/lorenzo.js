...
// The very problem is that `signer` must be able to sign eth-secp256k1 txs
export async function signEvmWithKeplr({
    ...
    // NOTE: either eth account or base account
    ...
    // NOTE: signer signDirect, get signatures.
    const { signature, signed } = await signer.signDirect(signerAddress, signDoc)

    // NOTE: encode tx bytes, what we want!
    return TxRaw.encode({
