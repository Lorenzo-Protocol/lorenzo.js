...
import { QueryTokenPairsRequest, QueryTokenPairsResponse, QueryTokenPairRequest, QueryTokenPairResponse, QueryParamsRequest, QueryParamsResponse, QueryBalanceRequest, QueryBalanceResponse } from "./query";
...
export class QueryClientImpl implements Query {
  ...
  tokenPairs(request: QueryTokenPairsRequest = {
    ...
    const promise = this.rpc.request("lorenzo.token.v1.Query", "TokenPairs", data);
  ...
  tokenPair(request: QueryTokenPairRequest): Promise<QueryTokenPairResponse> {
    ...
    const promise = this.rpc.request("lorenzo.token.v1.Query", "TokenPair", data);
    return promise.then(data => QueryTokenPairResponse.decode(new BinaryReader(data)));
  }
  params(request: QueryParamsRequest = {}): Promise<QueryParamsResponse> {
    const data = QueryParamsRequest.encode(request).finish();
    const promise = this.rpc.request("lorenzo.token.v1.Query", "Params", data);
    return promise.then(data => QueryParamsResponse.decode(new BinaryReader(data)));
  }
  balance(request: QueryBalanceRequest): Promise<QueryBalanceResponse> {
    const data = QueryBalanceRequest.encode(request).finish();
    const promise = this.rpc.request("lorenzo.token.v1.Query", "Balance", data);
