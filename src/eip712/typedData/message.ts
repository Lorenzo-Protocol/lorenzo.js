import { Type, createType, typeArrayAdjusted, typesAreEqual } from "./types";
...
const parseFieldAsArray = (params: ParseFieldParams) => {
  ...
  let inner = value;
  let typeForField: Type | undefined;
  ...
  if (isArray) {
    ...
    if (value.length === 0) {
      ...
      typeForField = createType(key, "string[]");
    }
  }
...
const addTypesToRoot = (root: JSONObject, key: string, newTypes: Type[]) => {
  let typedef: string = "";
...
export const addMsgTypesToRoot = (types: JSONObject, msg: JSONObject) => {
  if (!msg) {
