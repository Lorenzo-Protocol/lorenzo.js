...
const setupTest = async (signingMode: SigningMode) => {
    const signer = await LorenzoOfflineSigner.fromMnemonic(signingMode, mnemonic, 'lrz');
    const client = await LorenzoClient.connectWithEthSigner(rpcUrl, signer);
    return { client, signer };
};

const testSignAndBroadcast = async (client: LorenzoClient, signer: LorenzoOfflineSigner) => {
